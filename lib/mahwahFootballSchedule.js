...
export const MAHWAH_SCHEDULE_2026 = [
  // Result added 2026-09-04 sweep: confirmed W 28-12 via MaxPreps box score,
  // cross-checked against Pequannock's own MaxPreps schedule page (mutual
  // cross-listing, same technique used throughout these checklists) — both
  // sides agree exactly. Season-opening win, Mahwah's first game of 2026.
  { date: 'Thu, Sep 3', time: '7:00 PM', opponent: 'vs Pequannock', home: true, location: HOME_FIELD, result: { win: true, score: '28-12', usScore: 28, themScore: 12 } },
  // Result added 2026-09-12 sweep: confirmed L 21-22 via MaxPreps box score/
  // recap ("Dumont holds off Mahwah... 22-21 in Thursday's league outing").
  // Mahwah drops to 1-1 overall / 0-1 league; Dumont improves to 1-1 overall
  // / 1-0 league (their only league game played so far this season).
  { date: 'Thu, Sep 10', time: '6:00 PM', opponent: 'at Dumont', home: false, league: true, location: 'TBD — confirm venue (Dumont HS)', result: { win: false, score: '21-22', usScore: 21, themScore: 22 } },
  // Result added 2026-09-18 sweep (2nd run, post-game): confirmed L 14-20
  // via Mahwah's own MaxPreps box score/schedule page, independently
  // corroborated by Lakeland Regional's own MaxPreps game page (both agree:
  // Lakeland 20, Mahwah 14). Mahwah drops to 1-2 overall / 0-2 league;
  // Lakeland improves to 4-0 overall / 1-0 league (still unbeaten).
  { date: 'Fri, Sep 18', time: '7:00 PM', opponent: 'vs Lakeland Regional', home: true, league: true, location: HOME_FIELD, result: { win: false, score: '14-20', usScore: 14, themScore: 20 } },
  // Result added 2026-09-26 sweep: confirmed 7-7 TIE via MaxPreps' own recap
  // ("Football Recap: New Milford Ends Even with Mahwah" — infoSentience-
  // generated, no play-by-play detail) and the mirror recap on New
  // Milford's own side ("Football Recap: Mahwah Plays to a Tie"), mutual
  // cross-listing agreeing on the tie. New Milford moves to 3-1-1, Mahwah
  // to 1-2-1 — first tie either team has recorded this season, and the
  // first tie tracked anywhere on this site. Represented as
  // `win: null, tie: true` since the existing win:true/false schema has no
  // slot for a tie; lib/teamSchedule.js's getRecord/getLatestResult and
  // this page's standings-row computation were extended (backward-
  // compatibly) the same sweep to handle it. Not a league (American Red)
  // game — New Milford isn't a division member.
  { date: 'Fri, Sep 25', time: '7:00 PM', opponent: 'vs New Milford', home: true, location: HOME_FIELD, result: { win: null, tie: true, score: '7-7', usScore: 7, themScore: 7 } },
