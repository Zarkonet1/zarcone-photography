...
//
// FIXED MORE GENERALLY 2026-09-08: the same class of bug recurred without a
// scrimmage involved — volleyball's Sep 5 Old Bridge/Jefferson Township
// matches went unreported on MaxPreps for several days (genuinely no score
// posted anywhere publicly, not a data-entry gap — same "still unreported"
// pattern documented repeatedly in BRHS-SWEEP-LOG.md), so `result` stayed
// `null` past their game date. Since getNextGame/getNextMatch only checked
// `!g.result`, the site kept showing "last Saturday's game" as the
// upcoming/hero game days after it was actually played, instead of
// advancing to the real next game (Sep 8 vs. Mount St. Mary). `isPastDate`
// below fixes this at the source: a schedule entry only counts as "next"
// if its own date hasn't passed yet, regardless of whether a result has
// been entered. A past-dated, still-unresolved entry (like the two Sep 5
// matches) is simply skipped for Next Match/Game purposes — it still shows
// in the full schedule table as-is, and still gets picked up once a real
// result is entered, same as before. This does NOT change getRecord or
// getLastPlayedGame, which already only look at games that HAVE a result.

// Schedule dates carry no year ('Sat, Sep 5'), so this parses against
// whichever of {now's year, now's year - 1, now's year + 1} lands closest
// to `now` — handles both fall-sport pages (Aug-Nov, single calendar year)
// and wrestling's Nov-Mar schedule, which crosses a New Year's boundary.
// Returns null (never throws) for anything unparseable, and callers treat
// null as "can't tell, don't hide it" — fail open rather than silently
// dropping a game from Next Match/Game over a formatting quirk.
function parseGameDate(dateStr, now) {
  const cleaned = shortDate(dateStr);
  const year = now.getFullYear();
  const candidates = [year - 1, year, year + 1]
    .map((y) => new Date(`${cleaned}, ${y} 00:00:00`))
    .filter((d) => !isNaN(d));
  if (candidates.length === 0) return null;
  return candidates.reduce((closest, d) =>
    Math.abs(d - now) < Math.abs(closest - now) ? d : closest
  );
}

// True once a schedule entry's own date has fully passed (i.e. is before
// today), independent of whether `result` has been filled in yet. Compares
// by calendar day, not exact time, so a game happening later today (not yet
// played, no result yet) still counts as NOT past.
function isPastDate(dateStr, now) {
  const gameDate = parseGameDate(dateStr, now);
...
export function getRecord(games, fallback = 'Preseason · 0–0') {
  ...
  // `result.win === null` marks a tie (added 2026-09-26 for Mahwah's Sep 25
  // 7-7 tie vs. New Milford — the first tie recorded on any of these team
  // pages). Ties count toward neither wins nor losses; a 3rd number is
  // appended to the record only when at least one tie exists, so every
  // existing win/loss-only page's record string is unchanged.
  const wins = played.filter((g) => g.result.win === true).length;
  const losses = played.filter((g) => g.result.win === false).length;
  const ties = played.filter((g) => g.result.win == null).length;
  return ties > 0 ? `${wins}-${losses}-${ties}` : `${wins}-${losses}`;
}

export function getNextMatch(games, fallback = 'TBA', now = new Date()) {
  const upcoming = games.find((g) => !g.result && !g.scrimmage && !isPastDate(g.date, now));
...
export function getLatestResult(games, fallback) {
  ...
  const letter = played.result.win == null ? 'T' : played.result.win ? 'W' : 'L';
  return `${shortDate(played.date)}: ${letter} ${played.result.score} ${played.opponent}`;
}
...
export function getNextGame(games, now = new Date()) {
  return games.find((g) => !g.result && !g.scrimmage && !isPastDate(g.date, now)) || null;
}
