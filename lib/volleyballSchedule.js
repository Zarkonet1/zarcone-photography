...
export const SCHEDULE_2026 = [
  ...
  // REMOVED 2026-09-15 sweep: this array previously had a 'Wed, Sep 16 at
  // Delaware Valley' entry here. Fresh direct fetches of both BR's own
  // current MaxPreps schedule AND Delaware Valley's own current MaxPreps
  // schedule show no Sep 16 meeting between the two teams anywhere — the
  // date doesn't appear on either team's live page. Delaware Valley's own
  // schedule DOES show a Bridgewater-Raritan game, but on Wed, Oct 22, 5:30
  // PM (DV traveling to BR) — likely where this game actually moved to, but
  // that date isn't yet corroborated by a second source (BR's own MaxPreps
  // page, last updated Sep 10, hasn't picked it up yet), so it isn't added
  // here pending next sweep's re-check per the standing two-source rule.
