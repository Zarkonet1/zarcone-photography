...
export const SCHEDULE_2026 = [
  ...
  // Result added 2026-09-18 sweep: confirmed L 0-2 via MaxPreps match page
  // (Final: Hunterdon Central 2, Bridgewater-Raritan 0) and independently
  // corroborated by MaxPreps' own auto-generated recap ("Volleyball Recap:
  // Bridgewater-Raritan Picks Up Their First Blemish in League Play,"
  // published Sep 18, 2026, 1:33am) — both give BR's updated record as
  // 3-5 overall / 2-1 league, first league loss of the season. Note: the
  // team schedule/standings landing pages (both fetched fresh this same
  // sweep) still showed 3-4/2-0 with this match unplayed — a real
  // MaxPreps aggregation lag between the individual match page (updated)
  // and the schedule/standings pages (not yet updated as of this sweep).
  // Set-by-set score not available via NJRB or general search this run.
  { date: 'Tue, Sep 15', time: '5:30 PM', opponent: 'vs Hunterdon Central', home: true, league: true, location: HOME_VENUE, result: { win: false, score: '0-2', usScore: 0, themScore: 2 } },
  ...
  // Result added 2026-09-19 sweep: confirmed L 1-2 via MaxPreps match page
  // (og:description: "the Roxbury varsity volleyball team won their away
  // non-conference match against Bridgewater-Raritan... by a score of 2-1").
  // Set-by-set score not available via NJRB or general search this run.
  { date: 'Thu, Sep 17', time: '4:00 PM', opponent: 'vs Roxbury', home: true, location: HOME_VENUE, result: { win: false, score: '1-2', usScore: 1, themScore: 2 } },
  // Result added 2026-09-19 sweep: confirmed W 2-0 via MaxPreps match page
  // (og:description: "the Hillsborough varsity volleyball team lost their
  // away conference match against Bridgewater-Raritan... by a score of
  // 2-0"). Both this and the Roxbury result above were still showing as
  // unplayed on BR's own schedule/standings pages as of this sweep (schedule
  // last updated Sep 15) — same MaxPreps-internal aggregation lag already
  // documented 2026-09-18 (team page lags the individual match page); the
  // match pages themselves, fetched directly, are unambiguous. BR's overall
  // record is now 4-6 (league 3-1), confirmed via both match pages' team
  // record display and independently via the Skyland - Delaware East league
  // standings page (fetched fresh, same sweep).
  { date: 'Fri, Sep 18', time: '5:30 PM', opponent: 'vs Hillsborough', home: true, league: true, location: HOME_VENUE, result: { win: true, score: '2-0', usScore: 2, themScore: 0 } },
  // Added 2026-09-25 sweep: found via NJRB's BR team page, which had newly
  // caught up on several previously-unposted matches (had been lagging at
  // 3-4/2-2 through Sep 10 as of the 09-24 sweep). Confirmed via 3
  // independent sources: MaxPreps' own BR team page (Sep 22, away, L 0-2,
  // most-recent-match display), North Hunterdon's own MaxPreps schedule
  // page (Sep 22, home, W 2-0 — mutual cross-listing agrees), and NJRB's
  // box score (game/21266/: sets 25-16, 25-19; BR totals 11K/11A/32D/1B/6Ace,
  // led by Camille Hilton 11 digs, Brooke Krizan 10 assists). This is BR's
  // 2nd meeting with North Hunterdon this season (also played Sep 3, home,
  // W 2-0) — both are Skyland Delaware East league games per each team's
  // own MaxPreps standings page (BR's league record moved 3-1 -> 3-2 the
  // same day this match posted, confirming the league flag).
  { date: 'Tue, Sep 22', time: 'TBA', opponent: 'at North Hunterdon', home: false, league: true, location: 'TBD — confirm venue (North Hunterdon HS)', result: { win: false, score: '0-2', usScore: 0, themScore: 2 } },
