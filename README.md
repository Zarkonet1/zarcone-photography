...
## SEO Metadata — How It Works

Metadata is set per-page via Next.js `export const metadata` in each layout or page file.

**Root defaults** (`app/layout.jsx`):
- Title, description, canonical, og:image, twitter card — inherited by all pages
- JSON-LD `LocalBusiness` schema with telephone, sameAs (Instagram + Facebook), services, geo coordinates

**Per-page overrides** (each `layout.jsx` or `page.jsx`):
- Each page sets its own `title`, `description`, `openGraph`, and `twitter`
- `openGraph.images` and `twitter.card: 'summary_large_image'` are set on every page

**OG images by page:**
| Page | Image |
|------|-------|
| Homepage / default | `/photos/tz-headshot.jpg` |
| Sports | `/photos/i-s7zBdzk.jpg` |
| Portraits | `/photos/PORTRAIT-Zarcone-Photography-0002.jpg` |
| Events | `/photos/EVENT-Zarcone-Photography-28.jpg` |
| Design / About / FAQ / News | `/photos/tz-headshot.jpg` |
| Senior portrait landing | `/photos/i-rvRX82g.jpg` |
| Sports photographer landing | `/photos/i-s7zBdzk.jpg` |
| Event photographer landing | `/photos/EVENT-Zarcone-Photography-28.jpg` |

**robots.txt** — `public/robots.txt` — allows all crawlers, points to sitemap.

**Sitemap** — auto-generated by `app/sitemap.js`.
...
- **SEO audit & fixes (June 24, 2026)** — Full SEO pass. Added `og:image` and `twitter:card: summary_large_image` to all pages (root layout + every page/section layout). Fixed `og:title` mismatches on Portraits, Events, and Design. Fixed blank `telephone` and empty `sameAs` in root JSON-LD schema (now includes phone + Instagram + Facebook). Created `public/robots.txt` with sitemap pointer. Changes deployed to production.
